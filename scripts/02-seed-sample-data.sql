-- Sample data for LuxeAuction platform
...
((SELECT id FROM cars WHERE vin = '1F1234567890123456'), (SELECT id FROM users WHERE username = 'classiccarking'), '1967 Ferrari 275 GTB/4 - Matching Numbers', 'Rare opportunity to acquire this stunning Ferrari 275 GTB/4 with matching numbers and Ferrari Classiche certification.', 1200000.00, 2400000.00, 2400000.00, 'ETH', NOW() - INTERVAL '2 days', NOW() + INTERVAL '2 days', 'live', 47, 1247, true),

((SELECT id FROM cars WHERE vin = '2M1234567890123456'), (SELECT id FROM users WHERE username = 'luxuryautohaus'), '1955 Mercedes-Benz 300SL Gullwing', 'Iconic Gullwing in exceptional condition with original paint and comprehensive documentation.', 900000.00, 1800000.00, 1800000.00, 'ETH', NOW() - INTERVAL '1 day', NOW() + INTERVAL '1 day', 'live', 32, 892, true),

((SELECT id FROM cars WHERE vin = '3P1234567890123456'), (SELECT id FROM users WHERE username = 'speedster_mike'), '1970 Porsche 917K - Le Mans Legend', 'Authentic Le Mans race car with documented history and recent concours restoration.', 1600000.00, 3200000.00, 3200000.00, 'ETH', NOW() - INTERVAL '1 hour', NOW() + INTERVAL '4 days', 'live', 78, 2156, true),

((SELECT id FROM cars WHERE vin = '4A1234567890123456'), (SELECT id FROM users WHERE username = 'carbaron'), '1963 Aston Martin DB5', 'The quintessential Bond car in classic Silver Birch with matching numbers and service history.', 600000.00, 1200000.00, 1200000.00, 'ETH', NOW() - INTERVAL '30 minutes', NOW() + INTERVAL '1 hour', 'live', 23, 567, false),

((SELECT id FROM cars WHERE vin = '5D1234567890123456'), (SELECT id FROM users WHERE username = 'classiccarking'), '1969 Dodge Charger R/T', 'Numbers matching 440 Magnum R/T in solid condition with recent mechanical refresh.', 40000.00, 80000.00, 80000.00, 'ETH', NOW() - INTERVAL '45 minutes', NOW() + INTERVAL '2 hours', 'live', 18, 423, false),

((SELECT id FROM cars WHERE vin = '6P1234567890123456'), (SELECT id FROM users WHERE username = 'speedster_mike'), '1973 Porsche Carrera RS 2.7', 'Exceptional lightweight RS 2.7 with matching numbers and impeccable provenance.', 1050000.00, 2100000.00, 2100000.00, 'ETH', NOW() - INTERVAL '20 minutes', NOW() + INTERVAL '30 minutes', 'live', 31, 789, false);

-- Insert sample bids
INSERT INTO bids (auction_id, bidder_id, amount, currency, status) VALUES
-- Ferrari 275 GTB/4 bids
((SELECT id FROM auctions WHERE title LIKE '%Ferrari 275 GTB/4%'), (SELECT id FROM users WHERE username = 'luxuryautohaus'), 1200000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Ferrari 275 GTB/4%'), (SELECT id FROM users WHERE username = 'carbaron'), 1500000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Ferrari 275 GTB/4%'), (SELECT id FROM users WHERE username = 'speedster_mike'), 1800000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Ferrari 275 GTB/4%'), (SELECT id FROM users WHERE username = 'luxuryautohaus'), 2100000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Ferrari 275 GTB/4%'), (SELECT id FROM users WHERE username = 'carbaron'), 2400000.00, 'ETH', 'confirmed'),

-- Mercedes 300SL bids
((SELECT id FROM auctions WHERE title LIKE '%Mercedes-Benz 300SL%'), (SELECT id FROM users WHERE username = 'classiccarking'), 900000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Mercedes-Benz 300SL%'), (SELECT id FROM users WHERE username = 'speedster_mike'), 1200000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Mercedes-Benz 300SL%'), (SELECT id FROM users WHERE username = 'carbaron'), 1500000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Mercedes-Benz 300SL%'), (SELECT id FROM users WHERE username = 'classiccarking'), 1800000.00, 'ETH', 'confirmed'),

-- Porsche 917K bids
((SELECT id FROM auctions WHERE title LIKE '%Porsche 917K%'), (SELECT id FROM users WHERE username = 'carbaron'), 1600000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Porsche 917K%'), (SELECT id FROM users WHERE username = 'luxuryautohaus'), 2000000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Porsche 917K%'), (SELECT id FROM users WHERE username = 'classiccarking'), 2400000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Porsche 917K%'), (SELECT id FROM users WHERE username = 'carbaron'), 2800000.00, 'ETH', 'confirmed'),
((SELECT id FROM auctions WHERE title LIKE '%Porsche 917K%'), (SELECT id FROM users WHERE username = 'luxuryautohaus'), 3200000.00, 'ETH', 'confirmed');
